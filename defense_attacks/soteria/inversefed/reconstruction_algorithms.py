...
def reconstruction_costs(gradients, input_gradient, cost_fn='l2', indices='def', weights='equal'):
    ...
    # NOTE: If you want to use our attack please adapt the code here.
    remove_defense = True
    attack_idx = -4
    layer_idx = -1

    if remove_defense:
        layer_idx = indices[attack_idx]
        mask = 1 # (gradients[0][attack_idx] != 0).float()
        weights[attack_idx] = 0
    ...
    for trial_gradient in gradients:
        ...
        for i in indices:
            if i == layer_idx: # Custom masked layer attack
                if cost_fn == 'l2':
                    costs += (((trial_gradient[i] - input_gradient[i])*mask).pow(2)).sum()
                elif cost_fn == 'l1':
                    costs += (((trial_gradient[i] - input_gradient[i])*mask).abs()).sum()
                elif cost_fn == 'max':
                    costs += (((trial_gradient[i] - input_gradient[i])*mask).abs()).max()
                elif cost_fn == 'sim':
                    costs -= (trial_gradient[i] * (input_gradient[i]*mask)).sum() * weights[i]
                    pnorm[0] += trial_gradient[i].pow(2).sum() * weights[i]
                    pnorm[1] += (input_gradient[i]*mask).pow(2).sum() * weights[i]
