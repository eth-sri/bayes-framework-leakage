...
class GradientReconstructor():
    ...
    def _run_trial(self, x_trial, input_data, labels, reconstr_layer, gt, gt_optim, dryrun=False):
        x_trial_0 = (torch.rand(( self.num_images, 3, 1, 1 ), **self.setup) - 0.5) * 2
        x_trial_0.requires_grad = True
        x_trial_its = [x_trial_0]
        for i in range(5):
            dummy_seed_init = (torch.rand(( self.num_images, 3, 2**(i+1), 2**(i+1) ), **self.setup) - 0.5) * 0.1
            x_trial_i = torch.Tensor(dummy_seed_init).to(**self.setup).requires_grad_(True)
            x_trial_its.append( x_trial_i )
        ...
        if self.reconstruct_label:
            output_test = self.model(*x_trial_its,*(self.args+self.args_optim))
            labels = torch.randn(output_test.shape[1]).to(**self.setup).requires_grad_(True)

            if self.config['optim'] == 'adam':
                optimizer = torch.optim.Adam([*x_trial_its, labels, *self.args_optim], lr=self.config['lr'])
            elif self.config['optim'] == 'adamax':
                optimizer = torch.optim.Adamax([*x_trial_its, labels, *self.args_optim], lr=self.config['lr'])
            elif self.config['optim'] == 'sgd':  # actually gd
                optimizer = torch.optim.SGD([*x_trial_its, labels, *self.args_optim], lr=0.01, momentum=0.9, nesterov=True)
            elif self.config['optim'] == 'LBFGS':
                optimizer = torch.optim.LBFGS([*x_trial_its, labels, *self.args_optim])
            else:
                raise ValueError()
        else:
            if self.config['optim'] == 'adam':
                optimizer = torch.optim.Adam([*x_trial_its, *self.args_optim], lr=self.config['lr'])
            elif self.config['optim'] == 'adamax':
                optimizer = torch.optim.Adamax([*x_trial_its, *self.args_optim], lr=self.config['lr'])
            elif self.config['optim'] == 'rms':
                optimizer = torch.optim.RMSprop([*x_trial_its, *self.args_optim], lr=self.config['lr'])
            elif self.config['optim'] == 'sgd':  # actually gd
                optimizer = torch.optim.SGD([*x_trial_its, *self.args_optim], lr=0.01, momentum=0.9, nesterov=True)
            elif self.config['optim'] == 'LBFGS':
                optimizer = torch.optim.LBFGS([*x_trial_its, *self.args_optim])
            else:
                raise ValueError()
        ...
        try: 
            ...
            for iteration in range(max_iterations):
                closure = self._gradient_closure(optimizer, x_trial_its, input_data, labels, reconstr_layer)
                ...
                with torch.no_grad():
                    x_trial = x_trial_its[0].detach()
                    for i in range(5):
                        x_trial = x_trial.repeat_interleave(2, dim=2).repeat_interleave(2, dim=3) + x_trial_its[i+1].detach() *( 0.9 ** i )
                    # Project into image space
                    #if self.config['boxed']:
                    #    x_trial.data = torch.max(torch.min(x_trial, (1 - dm) / ds), -dm / ds)
                    if iteration % 100 == 0:
                        self.config['total_variation_smooth'] = min( self.config['total_variation_smooth']*self.config['total_variation_decay'], self.config['total_variation_smooth_min'] )
                        self.config['total_variation'] = min( self.config['total_variation']*self.config['total_variation_decay'], self.config['total_variation_min'] )
                    ...
                    if (iteration + 1 == max_iterations) or iteration % self.config['print_every'] == 0:
                        closure_print_text = f'It: {iteration}. Rec. loss: {rec_loss.item():2.8f}. TVS: {self.config["total_variation_smooth"]:.4f} TV:{self.config["total_variation"]:.4f}'
                        if not( gt is None ):
...
#                    if (iteration + 1) % 100 == 0:
                        ...
                        #    arg_optim.data = torch.randn(arg_optim.shape, **self.setup)
                        #    arg_optim.requires_grad = True
                        #x_trial.data += torch.randn_like(x_trial.data)*0.01
...
#                    break
    ...
    def _gradient_closure(self, optimizer, x_trials, input_gradient, label, reconstr_layer):
        ...
        def closure():
            ...
            x_trial = x_trials[0]
            for i in range(5):
                x_trial = x_trial.repeat_interleave(2, dim=2).repeat_interleave(2, dim=3) + x_trials[i+1] * ( 0.9 ** i )
